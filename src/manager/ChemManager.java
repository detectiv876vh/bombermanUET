    ...
    public void handleChem() {
        if (player.attackCooldown <= 0 && gp.kH.qPressed
            ...
            for (Entity[] monster : gp.monster) {
                if (monster != null && monster[gp.currentMap].getHitbox().intersects(attackArea)) {
                    monster[gp.currentMap].life -= 1;
                    monster[gp.currentMap].invincible = true;
                    monster[gp.currentMap].invincibleCounter = 0;
                }
            }
        }
        if (!player.moving) {
